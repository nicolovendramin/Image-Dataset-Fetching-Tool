...
def fetch_images_from_keyword(keyword,output_dir):
	...
	while True:
		...
		try:
			...
			for link in links:
				if counter >= tot:
					list = os.listdir(output_dir) # dir is your directory path
					number_files = len(list)
					if number_files >= tot:
						print(number_files)
						break
					else:
						left = tot - number_files
...
if __name__ == "__main__":
	...
	parser.add_argument('-m', '--multiple-strings', help = 'Multiple Keywords to Search, comma separated', required = False)
	...
	if (not args.search_string) and (not args.search_file) and (not args.multiple_strings):
		parser.error('Provide Either search string or path to file containing search strings')
	if args.output:
		output_dir = "images/" + args.output
	...
	if args.search_string:
		output_dir = "images/" + args.search_string
		if not os.path.exists(output_dir):
			os.makedirs(output_dir)
		fetch_images_from_keyword(args.search_string,output_dir)
	elif args.multiple_strings:
		keywords = args.multiple_strings.split(",")
		for keyword in keywords:
			output_dir = "images/" + keyword
			if not os.path.exists(output_dir):
				os.makedirs(output_dir)
			fetch_images_from_keyword(keyword,output_dir)
